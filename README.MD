...
#### 8.4.20 MySQL Database - Connect the database

        (venv3932) ...\DJANGO-CLONE-AMAZON-RENEWED\src> python -m pip install mysqlclient
        DATABASES = {
                'default': {
                        'ENGINE': 'django.db.backends.mysql',
                        'NAME': 'django_clone_amazon_renewed',
                        'USER': 'root',
                        'PASSWORD': '',
                        'HOST': 'localhost',
                        'PORT': 3306
...
#### 8.5.21 MySQL Database - Create tables to db

        modified:   README.MD
        (venv3932) ...\DJANGO-CLONE-AMAZON-RENEWED\src> python manage.py makemigrations
        (venv3932) ...\DJANGO-CLONE-AMAZON-RENEWED\src> python manage.py migrate

#### 8.6.22 MySQL Database - Show the created tables

        mysql> USE django_clone_amazon_renewed;
        Database changed
        mysql> SHOW tables;
        +---------------------------------------+
        | Tables_in_django_clone_amazon_renewed |
        +---------------------------------------+
        | auth_group                            |
...
#### 8.7.23 MySQL Database - Create superuser

        (venv3932) ...\DJANGO-CLONE-AMAZON-RENEWED\src> python manage.py createsuperuser
        ...
        | username | email                  |
        +----------+------------------------+
        | admin    | ingafter60@admin.com   | <-- modified
        +----------+------------------------+

        modified:   README.MD
        modified:   config/__pycache__/urls.cpython-39.pyc

#### 8.8.24 Model - Create CustomUser model

        .Import AbstracUser from django.contrib.auth.models
        .Create class CustomUser and use AbstractUser
        .Register AUTH_USER_MODEL to /settings.py
        .mysql> DROP DATABASE django_clone_amazon_renewed;
        ...
        +---------------------------------------+
        | adboard_customuser                    |
        | adboard_customuser_groups             |
        | adboard_customuser_user_permissions   |
        | auth_group                            |
        | auth_group_permissions                |
        | auth_permission                       |
        | django_admin_log                      |
        ...
        mysql> SELECT username, email from adboard_customuser;
        +----------+------------------------+
        | username | email                  |
        +----------+------------------------+
        | admin    | ingafter60@admin.com   | <-- modified
        ...
        modified:   README.MD
        new file:   app/adboard/migrations/0001_initial.py
        modified:   app/adboard/models.py
        modified:   config/settings.py

#### 8.9.25 Model - add user_type field to adboard_customuser model/tabel

        .There are 4 types of user: Admin(1), Staff(2), Merchant(3), Customer(4)
        .In CustomUser:
        > Define user_type_choices for the 4 types of user
        > Add user_type field to CustomUser
        >> Use Admin as default user
        >> max_length of the user_type field

        (venv3932) ...\DJANGO-CLONE-AMAZON-RENEWED\src> python manage.py makemigrations
        - Add field user_type to customuser
        (venv3932) ...\DJANGO-CLONE-AMAZON-RENEWED\src> python manage.py migrate
        ...
        | is_active    | tinyint(1)   | NO   |     | NULL    |                |
        | date_joined  | datetime(6)  | NO   |     | NULL    |                |
        | user_type    | varchar(225) | NO   |     | NULL    |                | <-- newly added field
        +--------------+--------------+------+-----+---------+----------------+
        12 rows in set (0.00 sec)

#### 8.10.26 Modified files name

        modified:   README.MD
        modified:   app/adboard/views.py
        modified:   config/urls.py
        renamed:    templates/adboard/home-adboard.html -> templates/adboard/home-admin.html
        renamed:    templates/adboard/login-adboard.html -> templates/adboard/login-admin.html
        renamed:    templates/adboard/shared/navbar-adboard.html -> templates/adboard/shared/navbar-admin.html
        renamed:    templates/base-adboard.html -> templates/base-admin.html
        modified:   templates/main/shared/navbar.html

#### 8.11.27 Login - Login process

        .In views, define loginAdminProcess
        .In templates, create login_admin_process.html
        .Add path to the urls
        .In login_admin page, add csrf token
        modified:   README.MD
        modified:   app/adboard/views.py
        modified:   config/urls.py
        modified:   templates/adboard/login-admin.html

#### 8.12.28 Login - Authenticate user login

        modified:   README.MD
        modified:   app/adboard/views.py
        modified:   templates/adboard/shared/navbar-admin.html
        modified:   templates/main/shared/navbar.html

#### 8.13.29 Login - Showing logged in username on the menu

        modified:   README.MD
        modified:   app/adboard/views.py
        modified:   config/urls.py
        renamed:    templates/adboard/home-admin.html -> templates/adboard/admin_home.html
        renamed:    templates/adboard/login-admin.html -> templates/adboard/admin_login.html
        modified:   templates/adboard/shared/navbar-admin.html
        modified:   templates/main/shared/navbar.html

#### 8.14.30 Logout - Create logout method, url path and menu logout
